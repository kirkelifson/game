...
int CHudCredits::KeyInput(int down, ButtonCode_t keynum, const char *pszCurrentBinding)
{
    if (keynum == MOUSE_LEFT)
    {
        if (down >= 1)
            m_flCurrentScrollTime = m_flScrollTime * 2;
        else
            m_flCurrentScrollTime = m_flScrollTime;
        return 0;
    }
    else
    {
...
void CHudCredits::DrawOutroCreditsName(void)
{
    ...
    for (int i = 0; i < m_CreditsList.Count(); i++)
    {
        ...
        if (i == m_CreditsList.Count() - 1)
        {
            ...
            {
                pCredit->flYPos -= gpGlobals->frametime * ((float)g_iCreditsPixelHeight / m_flCurrentScrollTime);
            ...
            else
            {
                if (m_flFadeTime <= gpGlobals->curtime)
                {
                    if (m_Alpha > 0)
                    {
                        m_Alpha -= gpGlobals->frametime * (m_flCurrentScrollTime * 2);
